...
bool is_prime(mpz_t n, uint64_t iters){
	...
	mpz_set_ui(result, 2);
	while(true){
		mpz_pow_ui(result, 2, s);
		mpz_mul(result, r);
		mpz_add_ui(result, 1);
		if(mpz_cmp_(n, result) == 0){
			break;
		}
		mpz_fdiv_ui(result, r, 2);
		mpz_set(r, result);
		mpz_add(result, s, 1);
		mpz_set(s, result);
	}
	...
	for(uint64_t i = 1, i < iters; i += 1){
		...
		mpz_t a;
		mpz_init(a);
		mpz_set(a, gmp_urandomm_ui(state, n_rand));
		mpz_add_ui(result, a, 2);
		mpz_set(a, result);		
		mpz_t y;
		pow_mod(y, a, r, n);
		mpz_sub_ui(result, n, 1);
		if((mpz_cmp_ui(y, 1) < 0 || mpz_cmp_ui(y, 1) > 0) && (mpz_cmp_ui(y, result) < 0 || mpz_cmp_ui(y, result) > 0)){
			mpz_t j;
			mpz_init_set_ui(j, 1);
			mpz_sub_ui(result2, s, 1);
			while((mpz_cmp(j, result2) < 0 || mpz_cmp(j, result2) == 0) && (mpz_cmp(y, result) < 0 || mpz_cmp(y, result) > 0)){
				pow_mod(result3, y, 2, n);
				mpz_set(y, result3);
				if(mpz_cmp_ui(y, 1) == 0){
					return_bool = false;
					break;
